                        ...
                        <a v-bind:href="'/profiles/'+ user.id">
                            <div class="flow-root bg-gray-100 p-4 rounded-lg text-xl"> 
                                ...
                                <div class="flex gap-x-4 float-right">
                                    <div class="py-2 item-right w-12 h-12" @click.prevent="acceptFriendRequest(user)">
                                    ...
                                    <div class="py-2 item-right" @click.prevent="declineFriendRequest(user)">
        ...
        methods: {
            acceptFriendRequest(userToAccept) {
                ...
                axios.post("/api/acceptfriendrequest", 
                    {
                        user: this.user, 
                        userToAccept: userToAccept.id, 
                    })
                    .then((response) => {
                        console.log(response.data);
                    });

                var userToSplice = this.users.indexOf(userToAccept)
                this.users.splice(userToSplice, 1)
            ...
            declineFriendRequest(userToDecline) {
                ...
                axios.post("/api/declinefriendrequest", 
                    {
                        user: this.user, 
                        userToDecline: userToDecline.id, 
                    })
                    .then((response) => {
                        console.log(response.data);
                    });

                var userToSplice = this.users.indexOf(userToDecline)
                this.users.splice(userToSplice, 1)
            }
        }
    }
